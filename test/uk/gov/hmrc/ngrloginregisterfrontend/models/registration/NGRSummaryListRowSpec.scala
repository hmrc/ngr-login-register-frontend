...
class NGRSummaryListRowSpec extends TestSupport {
  ...
  "buildSummaryListRow" when {
    ...
    "will generate a SummaryListRow with a value if the value is set in CheckYourAnswersRow" in {
      ...
      val result = NGRSummaryListRow.summarise(voaSummaryListRow)
      result shouldBe SummaryListRow(Key(Text("Name"), "govuk-!-width-one-third"), Value(HtmlContent("<span id=\"checkyouranswers.name-id\">Jimley Jackson</span>")), "", None)
    }
    ...
    "will generate a SummaryListRow with a separated lines if the value is set as multiple strings in CheckYourAnswersRow" in {
      ...
      val result = NGRSummaryListRow.summarise(voaSummaryListRow)
      result shouldBe SummaryListRow(Key(Text("Address"), "govuk-!-width-one-third"), Value(HtmlContent("<span id=\"checkyouranswers.address-id\">Line1</br>Line2</span>")), "", None)
    }
    ...
    "will generate a SummaryListRow with an Action" in {
      ...
      val result = NGRSummaryListRow.summarise(voaSummaryListRow)
      result shouldBe SummaryListRow(Key(Text("Address"), "govuk-!-width-one-third"), Value(HtmlContent("<span id=\"checkyouranswers.address-id\">Line1</br>Line2</span>")), "", Some(Actions("", List(ActionItem("url", Text("Change"), None, "", Map("id" -> "id"))))))
    }
