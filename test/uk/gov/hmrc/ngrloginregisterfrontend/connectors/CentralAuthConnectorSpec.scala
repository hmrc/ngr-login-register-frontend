  ...
  "Calling the token search api" when {
    ...
    "a 400-499 response is returned" should {
      "return an ErrorResponse" in {
        ...
        setupMockHttpV2Post(s"${mockConfig.centralAuthServerUrl}/centralised-authorisation-server/token/search")(errorResponse)
        val result = centralAuthConnector.getTokenAttributesResponse(gnapToken)
    ...
    "a 500-599 response is returned" should {
      "return an ErrorResponse" in {
        setupMockHttpV2FailedPost(s"${mockConfig.centralAuthServerUrl}/centralised-authorisation-server/token/search")
        val result = centralAuthConnector.getTokenAttributesResponse(gnapToken)
