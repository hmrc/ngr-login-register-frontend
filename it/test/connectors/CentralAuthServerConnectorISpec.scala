...
import uk.gov.hmrc.ngrloginregisterfrontend.connectors.centralauth.CentralAuthConnector
    ...
    "calling .getTokenAttributesResponse()" when {
      "sending a request" should {
        "return a successful response" in {
          WiremockHelper.stubPostWithHeaders(s"/centralised-authorisation-server/token/search",OK, tokenAttributesResponseJson,"User-Agent","ngr-login-register-frontend")
        ...
        "return an error when the request fails" in {
          WiremockHelper.stubPostWithHeaders(s"/centralised-authorisation-server/token/search", INTERNAL_SERVER_ERROR, "Call to Central Auth Server Failed","User-Agent","ngr-login-register-frontend")
